...
import { Printer, Calendar, ShieldCheck, Share2, Check, Pill, ClipboardList, Info, CheckCircle2, Clock, AlertTriangle } from 'lucide-react';
...
export const DoctorReportPage: React.FC = () => {
  ...
  // Compute summary stats for the report - ALL 15 indicators are active and included
  const reportStats = useMemo(() => {
    ...
    for (const key of allKeys) {
      ...
      if (values.length > 0 && base) {
        const avg = values.reduce((a, b) => a + b, 0) / values.length;
        const dev =
          base.ewmaMean !== 0
            ? Math.round(((avg - base.ewmaMean) / base.ewmaMean) * 100)
            : 0;
        const z = calculateZScore(avg, base.ewmaMean, base.ewmaStd);
        ...
        // Include baseline even if no period metric recorded yet for complete visibility
        stats.push({
          key,
          label: def.label,
          unit: def.unit,
      ...
      <div className="no-print space-y-4">
        ...
        <div className="bg-white rounded-3xl p-4 sm:p-5 border border-comus-sand-light/20 shadow-soft flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div className="text-[11px] text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-xl border border-emerald-200 flex items-center gap-1 font-medium">
            ...
            <span>Tüm 15 Gösterge Aktif & Rapora Dahil</span>
          </div>
        </div>
      </div>

      {/* Printable Report Sheet Document */}
      <div className="bg-white rounded-3xl p-6 sm:p-10 border border-comus-sand-light/30 shadow-soft-lg print:border-none print:shadow-none print:p-0 space-y-7">
        {/* Report Header */}
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-comus-surface border border-comus-sand-light/20">
          ...
          <p className="text-xs sm:text-sm text-comus-sand-dark leading-relaxed">
            {userProfile.name} adlı kullanıcının son {selectedRange} günlük cihaz içi etkileşimleri, yazım temposu, hata düzeltme oranları, fiziksel hareketlilik ve sirkadiyen dinlenme pencereleri EWMA kişisel baz hattı ile boylamsal olarak karşılaştırılmıştır. Sistemdeki tüm 15 davranışsal gösterge eksiksiz olarak analize dahil edilmiştir. Bu veriler klinik tanı içermemekte olup uzman hekim ve terapist değerlendirmesine destek amacıyla sunulmuştur.
          </p>
        </div>

        {/* 2. Medication Usage Instructions (İlaç Kullanım Talimatı) */}
        <div className="p-5 rounded-2xl bg-teal-50/60 border border-teal-200/90 space-y-3.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            ...
            <div className="p-3 bg-white/90 rounded-xl border border-teal-200/60 space-y-1">
              ...
              <p className="text-teal-900/90 leading-relaxed text-[11.5px]">
                Sedatif (uyku verici) etkili ilaçları gece dinlenme penceresinden 30-45 dk önce alınız. Alkol kullanımından kaçınınız; kafein miktarını sınırlayınız.
              </p>
            </div>
          </div>
        </div>

        {/* 3. Daily Medication Intake Schedule Table (Günlük Kullanım Tablosu) */}
        <div className="space-y-3">
          ...
          <div className="overflow-x-auto rounded-2xl border border-comus-sand-light/30">
                    ...
                    return (
                      <tr key={med.id} className={isTakenToday ? 'bg-teal-50/40' : 'hover:bg-comus-surface/50'}>
                        ...
                        <td className="p-3 text-comus-navy">
                          {med.notes || 'Duygudurum ve sirkadiyen ritim regülasyonu (Hekim protokolü)'}
                        </td>
        ...
        {medications.length > 0 && (
          <div className="space-y-3">
            ...
            {medImpactReports.map((ir, idx) => (
              <div key={idx} className="p-4 rounded-2xl bg-teal-50/50 border border-teal-200/80 space-y-3">
                ...
                {/* Delta Table */}
                <div className="overflow-x-auto rounded-xl border border-teal-200/60 bg-white">
        ...
        {/* 5. Complete Metrics Table (All 15 Indicators Active) */}
        <div>
                    ...
                    return (
                      <tr key={st.key} className={isAnomaly ? 'bg-rose-50/40' : 'hover:bg-comus-surface/50'}>
                        ...
                        <td className="p-3 text-right font-mono tabular-nums font-medium whitespace-nowrap">
                          <span className={st.deviationPercent > 0 ? 'text-amber-700' : 'text-indigo-700'}>
